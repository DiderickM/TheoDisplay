...
class ImageScroller(SampleBase):
    ...
    def run(self):
        ...
        while True:
            home = '/home/pi/rpi-rgb-led-matrix/bindings/python/samples/'
            theo_t_32_x_64 = Image.open(home + 'test2_copy.png').convert('RGB').rotate(-90, expand=True)
            theo_198x64 = Image.open(home + "THEO_GROOT.png").rotate(-90, expand=True)
            t_42x64 = Image.open(home + "T.png").rotate(-90, expand=True)
            h_42x64 = Image.open(home + "H.png").rotate(-90, expand=True)
            e_42x64 = Image.open(home + "E.png").rotate(-90, expand=True)
            o_42x64 = Image.open(home + "O.png").rotate(-90, expand=True)
            #
            o_resized = o_42x64.resize((60,30))


            for opacity in range(5,100,2):
                enhancer = ImageEnhance.Brightness(o_resized)
                self.matrix.SetImage(enhancer.enhance(opacity/100), 2, 1)
                time.sleep(0.04)
